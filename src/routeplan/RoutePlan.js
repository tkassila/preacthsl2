...
const RoutePlan = (props) => {
  const [routeplanclicked, setRouteplanclicked] = useState(
    props.showShowAllLegs == null ? false : props.showShowAllLegs
  );
  const [showShowAllLegs, setShowShowAllLegs] = useState(
    props.showShowAllLegs == null ? false : props.showShowAllLegs
  );
  /*
    constructor(props) {
  ...
  const planClicked = (event) => {
    ...
    if (Config.bDebug) console.log("planClicked() 2");
  };
  ...
  let starttime = null;
  if (legname == "WALK")
    starttime = StaticFunctions.getToStringFromEndTime(
      leg.startTime,
      leg.arrivalDelay
    );
  else
  ...
  } else if (leg != null && leg.route == null && props.plan.legs.length > 1) {
    if (Config.bDebug)
      console.log(
        "leg != null && leg.intermediateStops == null && props.plan.legs.length > 1"
      );
  ...
  if (leg == null) return null;
  if (Config.bDebug) console.log("not null"); // href={"routeplan" +this.props.id}
  let planlink = (
    <a
      className={"a" + cssDark}
      id={"planlink" + props.id}
      href=""
      onClick={planClicked}
    >
      {legname} Lähtöaika {starttime} Etäisyys{" "}
      {StaticFunctions.getRoundedMeterDistance(leg.distance)}
    </a>
  );
  if (Config.bDebug) {
  ...
  if (!routeplanclicked)
    return (
      <li>
        <div className={"div" + cssDark}>{planlink}</div>
      </li>
    );
  else {
    if (Config.bDebug) console.log("else 1");
